...
import 'package:roshini/model/complaint_list_model.dart';
import 'package:roshini/model/profile_model.dart';
import 'package:roshini/model/registration_list_model.dart';
...
//  static String Id = ''.toString();
  ...
  //add services
  static addServicesApi(String Name,) async {
    try {
  ...
  static complaintListOpenApi() async {
    var url = 'https://jkroshini.com/api/RegistrationComplain/GetComplainListOpen';
    try {
      http.Response r = await http.get(Uri.parse(url));
      if (r.statusCode == 200) {
        ComplaintListOpenModel complainListOpenModel = complaintListOpenModelFromJson(r.body);
  ...
  static complaintListCloseApi() async {
    var url = 'https://jkroshini.com/api/RegistrationComplain/GetComplainListClose';
    try {
      http.Response r = await http.get(Uri.parse(url));
      if (r.statusCode == 200) {
        ComplaintListCloseModel complainListCloseModel = complaintListCloseModelFromJson(r.body);
  ...
  static complaintListPendingApi() async {
    ...
    try {
      http.Response r = await http.get(Uri.parse(url));
      if (r.statusCode == 200) {
        ComplaintListPendingModel complainListModel = complaintListPendingFromJson(r.body);
  ...
  // Complain Register
  static ComplainRegister(var Name,var Mobile,var Address,var dropdownValue,var ProductId,) async {
    Map map={
      "Name": Name,
      "Mobile": Mobile,
      "Address": Address,
    ...
    final String url = "https://jkroshini.com/api/RegistrationComplain/AddComplain";
    http.Response r = await http.post(
      Uri.parse(url,),
      headers: {
        "content-type" : "application/json",
  ...
  static profileApi() async {
    var url = 'https://jkroshini.com/api/Registration/GetProfile/sdafadsfdsaf3452345243';
    try {
      http.Response r = await http.get(Uri.parse(url));
      if (r.statusCode == 200) {
  ...
  // signup register
  static registerApi(var Name,var Number,var ShopName,var Address,var AadharName,Servicetype,var Password,img) async {
  ...
  static SliderBannerApi() async {
    var url = 'https://jkroshini.com/api/Registration/GetBanner/1';//baseUrl + 'api/AdminApi/BannerImage';
    try {
      http.Response r = await http.get(Uri.parse(url));
      if (r.statusCode == 200) {
        SliderListModel sliderbanerlist = sliderListModelFromJson(r.body);
        print('sliderBannerApi: ${sliderbanerlist.result?[0].image.toString()}');
  ...
  static RegistrationListApi()async {
    ...
    try {
      http.Response r = await http.get(Uri.parse(url));
      if (r.statusCode == 200) {
        RegistrationListModel registrationListModel = registrationListFromJson(r.body);
        print('registrationListModel: ${registrationListModel.result?[0].name}');
